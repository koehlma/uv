...
class Pipe(stream.Stream):
    """
    Pipe handles provide an abstraction of local domain sockets on Unix
    ...
    def __init__(self, loop=None, ipc=False):
        ...
        :param ipc:
            pipe should support inter process communication or not
    ...
    def pending_type(self):
        """
        Type of first pending stream. This returns a subclass of
        :class:`uv.Stream`.
    ...
    def sockname(self):
        ...
        :raises uv.HandleClosedError: handle
            has already been closed or is closing
    ...
    def bind(self, path):
        ...
        :raises uv.UVError:
            error while binding pipe to `path`
        :raises uv.HandleClosedError:
            handle has already been closed or is closing
    ...
    def connect(self, path, on_connect=None):
        ...
        :type path:
            unicode
        :type on_connect:
            ((uv.ConnectRequest, uv.StatusCode) -> None) |
