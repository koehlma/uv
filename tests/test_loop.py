...
class TestLoop(unittest.TestCase):
    ...
    def test_close(self):
        ...
        self.assertRaises(uv.ClosedStructure, uv.Async, loop=self.loop)
        self.assertRaises(uv.ClosedStructure, uv.Check, loop=self.loop)
        self.assertRaises(uv.ClosedStructure, uv.Idle, loop=self.loop)
        self.assertRaises(uv.ClosedStructure, uv.Pipe, loop=self.loop)
        self.assertRaises(uv.ClosedStructure, uv.Poll, 0, loop=self.loop)
        self.assertRaises(uv.ClosedStructure, uv.Prepare, loop=self.loop)
        self.assertRaises(uv.ClosedStructure, uv.Process, ['python'], loop=self.loop)
        self.assertRaises(uv.ClosedStructure, uv.Signal, loop=self.loop)
        self.assertRaises(uv.ClosedStructure, uv.TCP, loop=self.loop)
        self.assertRaises(uv.ClosedStructure, uv.Timer, loop=self.loop)
        self.assertRaises(uv.ClosedStructure, uv.TTY, 0, loop=self.loop)
        self.assertRaises(uv.ClosedStructure, uv.UDP, loop=self.loop)
